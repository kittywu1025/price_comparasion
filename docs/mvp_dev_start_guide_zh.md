...
- `home.html`：新手教程、优惠信息、快捷入口；不显示开发者专用入口。
- `products.html`：商品清单、搜索、商品详情、编辑和删除价格记录。
- `add.html`：录入商品价格、扫码、拍照/相册、快速填入、重复商品提示；快速填入识别到的价格默认按税前价格带入。
- `stores.html`：店铺列表、新增、编辑、删除、点击店铺进入该店商品。
- `profile.html`：个人贡献、用户名弹窗修改、反馈入口、开发者模式；管理员打开开发者模式后显示日语读音标注入口。
...
- 单位默认规格：`g = 1000`、`ml = 1000`、`个 = 1`、`pack = 1`；录入或修正时输入框只显示 placeholder，真正提交时才补默认值。

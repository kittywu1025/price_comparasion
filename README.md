...
- 商品录入：移动端优先布局，支持扫码条码、拍照、相册上传、上传前压缩图片、最多保留 4 张图片、快速填入、店铺输入匹配、税后价自动计算、限时优惠开关；快速填入识别出的价格默认写入税前价格，税后价格继续按当前税率自动计算；扫码在 Safari 不支持原生识别时会自动使用本地 ZXing 前端解码组件。
...
- 店铺情报投稿：`store.html` 店铺详情页支持普通用户上传店内海报或公告；弹窗只保留图片必填、备注可选、活动结束日期可选，支持拍照上传、相册多选和一次上传多张图片；图片会在前端压缩为 JPEG 后保存，多图优先保存在 `images` / `images_json`，旧的单图 `imageData` / `image_data` 仍兼容显示；普通用户只能管理自己的投稿，管理员可管理全部；卡片显示图片、备注、上传者、上传日期和活动结束日期，并对过期或超过 30 天的旧情报给出提示。
